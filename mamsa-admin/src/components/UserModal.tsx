...
export default function UserModal({ isOpen, onClose, onSave, editingItem }: UserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingItem && formData.role === 'super_admin' && !password) {
      finalPassword = 'adminmamsa';
      setPassword(finalPassword);
      setConfirmPassword(finalPassword);
    }

    if (!editingItem && (!finalPassword || finalPassword !== confirmPassword)) {
      if (formData.role === 'super_admin' && !password) {
        // Auto-set default password for super_admin
        finalPassword = 'adminmamsa';
        setPassword(finalPassword);
        setConfirmPassword(finalPassword);
      } else {
    ...
    try {
      const result = await onSave({
        user: formData,
        password: editingItem ? undefined : (finalPassword || password),
      });
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="password" className="block text-sm font-medium text-gray-700 mb-2">
                        Password {formData.role === 'super_admin' ? '(default: adminmamsa)' : '*'}
                      </label>
                      <div className="relative">
                        <input
                          ...
                          placeholder={formData.role === 'super_admin' ? 'Leave blank for default (adminmamsa)' : 'Enter password...'}
                        />
                        <button
                          type="button"
                    ...
                    <div>
                      <label htmlFor="confirmPassword" className="block text-sm font-medium text-gray-700 mb-2">
                        Confirm Password {formData.role === 'super_admin' ? '(default: adminmamsa)' : '*'}
                      </label>
                      <input
                        type={showPassword ? "text" : "password"}
                        id="confirmPassword"
                        ...
                        required={formData.role !== 'super_admin'}
                        className="block w-full border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 placeholder-gray-500 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                        placeholder={formData.role === 'super_admin' ? 'Leave blank for default (adminmamsa)' : 'Confirm password...'}
                      />
                    </div>
                  </div>
                </div>
