...
import { parseSocialLinksInput } from './student-form-utils';
...
export default function CreateStudentModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await adminRequest<unknown>('/api/admin/skilled-students', {
        method: 'POST',
        body: JSON.stringify({
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4 pt-8">
      <div className="relative my-4 w-full max-w-3xl rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between rounded-t-xl border-b border-gray-200 bg-gradient-to-r from-emerald-50 to-teal-50 p-6">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">Add skilled student</h3>
            <p className="text-sm text-gray-500">Create a directory listing; add payments on the detail page.</p>
          </div>
          <button
            type="button"
            onClick={handleClose}
            className="rounded-lg p-2 text-gray-400 transition hover:bg-white hover:text-gray-600"
            aria-label="Close"
          >
        ...
        <form onSubmit={handleSubmit} className="max-h-[70vh] space-y-4 overflow-y-auto p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-2 sm:col-span-2">
              <input id="create-featured" type="checkbox" checked={isFeatured} onChange={(e) => setIsFeatured(e.target.checked)} />
              <label htmlFor="create-featured" className="text-sm text-gray-700">
                Featured listing
              </label>
            </div>
            <div className="flex items-center gap-2 sm:col-span-2">
              <input id="create-active" type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} />
