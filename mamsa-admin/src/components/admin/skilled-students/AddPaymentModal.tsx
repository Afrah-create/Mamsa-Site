...
export default function AddPaymentModal({ isOpen, studentId, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    setExpiryDate(oneYearFromToday());
    setPaymentMethod('');
    setTransactionRef('');
    setStatus('pending');
    setNotes('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!paymentDate || !expiryDate) {
      setFormError('Payment date and expiry date are required.');
      return;
    }

    setLoading(true);
    try {
      await adminRequest<unknown>(`/api/admin/skilled-students/${studentId}/payments`, {
        method: 'POST',
        body: JSON.stringify({
          amount: amt,
