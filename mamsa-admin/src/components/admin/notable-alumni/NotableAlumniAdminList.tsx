...
export default function NotableAlumniAdminList() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const data = await adminRequest<ListPayload>(`/api/admin/notable-alumni?${q.toString()}`);
      setRows(data?.items ?? []);
      ...
      console.error(e);
      setRows([]);
      showToast('Failed to load notable alumni.', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await adminRequest(`/api/admin/notable-alumni/${Number(deleteTarget.id)}`, { method: 'DELETE' });
  ...
  return (
    <AdminLayout user={user}>
      ...
      <CreateAlumniModal
        ...
        onSuccess={() => {
          showToast('Alumni created.', 'success');
          afterMutation();
        }}
      />
      ...
      <ConfirmModal
        ...
        title="Delete alumni"
        message={deleteTarget ? `Remove ${String(deleteTarget.full_name ?? '')}?` : ''}
        confirmText="Delete"
      ...
      <div className="space-y-6 p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Notable alumni</h1>
            <p className="mt-1 text-sm text-gray-500">Highlight graduates and their paths.</p>
          </div>
          <button
            type="button"
                  ...
                  rows.map((r) => {
                    ...
                    return (
                      <tr key={id} className="hover:bg-gray-50/80">
                        ...
                        <td className="px-4 py-3 text-gray-600">{r.graduation_year != null ? String(r.graduation_year) : '—'}</td>
                        <td className="px-4 py-3 text-gray-600">{String(r.current_position ?? '—')}</td>
